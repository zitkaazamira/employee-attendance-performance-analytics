...
def render_bar(label, value, maximum, decimals=0):
    numeric_value = float(value)
    numeric_maximum = float(maximum) if maximum else 0

    width = (
        0
        if numeric_maximum == 0
        else numeric_value / numeric_maximum * 100
    )
    ...
    st.html(
        ...
        ">
            <div style="display:flex;justify-content:space-between;gap:10px;margin-bottom:8px;">
                ...
                <div style="color:#934c70;font-size:13px;font-weight:800;">{display_value}</div>
            </div>
